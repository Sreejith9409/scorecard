...
class Student
  ...
  def Student.get_student_report
    Student.construct_data
    Student.export_to_csv
  end

  private

  def Student.construct_data
    file_data = File.read("input.txt").split
    @students ||= []
    file_data.each do |f_data|
      data = f_data.split(",")
      id = data[0]
      s1 = Student.get_student_mark(data[1])
      s2 = Student.get_student_mark(data[2])
      s3 = Student.get_student_mark(data[3])
      s4 = Student.get_student_mark(data[4])
      total = (s1 + s2 + s3 + s4)
      @students << [data[0], s1, s2, s3, s4, total, Student.calculate_grade(total)]
    end
    Student.find_category_of_students
  end

  def Student.get_student_mark(data)
    data.to_s.split('-')[1].to_f
  end

  def  Student.calculate_grade(total)
    grade = if total >= 340
      'A'
  ...
  def Student.find_category_of_students
    avg_a, avg_b, avg_c = Student.calculate_grade_avg
    @below_avg_students = @above_avg_students = 0
    @students.each do |s|
      case s[6]
      when 'A' then s << Student.compare_average(s[5], avg_a)
      when 'B' then s << Student.compare_average(s[5], avg_b)
      when 'C' then s << Student.compare_average(s[5], avg_c)
      end
    end
  end

  def Student.compare_average(total, grade_average)
  ...
  def Student.calculate_grade_avg
